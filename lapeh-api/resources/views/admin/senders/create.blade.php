...
@section('title', __('admin.add_sender'))
...
<div style="max-width:700px;">
    ...
    <div class="card">
        <div class="card-head"><h3 class="sora" style="font-size:15px;font-weight:700;">{{ __('admin.new_sender') }}</h3></div>
        <form method="POST" action="{{ route('admin.senders.store') }}" style="padding:24px;" x-data="{ type: '{{ old('type', 'individual') }}' }">
            @csrf
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div class="form-group">
                    ...
                    <select name="type" x-model="type" class="form-input form-select">
                        <option value="individual">{{ __('admin.individual') }}</option>
                        <option value="business">{{ __('admin.business') }}</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('admin.contact_name') }}</label>
                    <input type="text" name="user_name" value="{{ old('user_name') }}" required class="form-input">
                </div>
                <div class="form-group">
            ...
            <div x-show="type === 'business'" style="border-top:1px solid var(--line);margin:8px 0 16px;padding-top:16px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        ...
                        <input type="text" name="business_name" value="{{ old('business_name') }}" class="form-input">
                    </div>
                    <div class="form-group">
                        <label class="form-label">{{ __('admin.business_category') }}</label>
                        <input type="text" name="business_category" value="{{ old('business_category') }}" class="form-input">
                    </div>
                    <div class="form-group" style="grid-column:span 2;">
                        <label class="form-label">{{ __('admin.contact_person') }}</label>
                        <input type="text" name="contact_person_name" value="{{ old('contact_person_name') }}" class="form-input">
                    </div>
                </div>
            </div>
            ...
            <div style="border-top:1px solid var(--line);margin:8px 0 16px;padding-top:16px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="default_pickup_address" value="{{ old('default_pickup_address') }}" class="form-input">
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    <div class="form-group">
                        <label class="form-label">{{ __('admin.latitude') }}</label>
                        <input type="number" name="default_pickup_lat" value="{{ old('default_pickup_lat') }}" step="any" class="form-input" placeholder="25.2048">
                    </div>
                    <div class="form-group">
                        <label class="form-label">{{ __('admin.longitude') }}</label>
                        <input type="number" name="default_pickup_lng" value="{{ old('default_pickup_lng') }}" step="any" class="form-input" placeholder="55.2708">
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex;gap:10px;justify-content:flex-end;">
                ...
                <button type="submit" class="btn btn-primary">{{ __('admin.create_sender') }}</button>
            </div>
        </form>
    </div>
</div>
