...
<div class="card">
    <table class="table">
        <thead><tr><th>{{ __('admin.sender') }}</th><th>{{ __('admin.order') }}</th><th>{{ __('admin.type') }}</th><th>{{ __('admin.status') }}</th><th>{{ __('admin.created') }}</th><th></th></tr></thead>
        <tbody>
            @forelse($complaints as $complaint)
            <tr>
                <td style="font-size:13.5px;font-weight:600;">{{ $complaint->sender?->displayName() }}</td>
